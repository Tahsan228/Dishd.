...
export function DiscoveryFeed({ kitchens, dishes, visited, initialLocation, today, now, unavailable }: Props) {
  ...
  return <>
    ...
    <div className="mt-6 flex snap-x gap-4 overflow-x-auto pb-2 md:grid md:grid-cols-3 md:overflow-visible">
      <Link href="/rewards" className="relative min-w-[86%] shrink-0 snap-start overflow-hidden rounded-2xl bg-forest p-5 text-cream md:min-w-0 md:p-6"><Gift className="h-6 w-6 text-brass" aria-hidden /><p className="mt-4 text-xs font-medium text-cream/75">A little thank-you for eating local</p><h2 style={{ fontFamily: "var(--font-sans)", fontWeight: 600 }} className="mt-2 text-xl leading-snug">Your next meal could cost $5 less</h2><p className="mt-3 text-sm text-cream/85">Redeem 250 points for a $5 credit. $15 food minimum.</p><span className="mt-4 inline-flex items-center gap-2 text-sm font-medium">Explore rewards <ArrowRight className="h-4 w-4" aria-hidden /></span></Link>
      <button type="button" onClick={selectFamily} className="min-w-[86%] shrink-0 snap-start rounded-2xl border border-brass/25 bg-brass/10 p-5 text-left text-forest md:min-w-0 md:p-6"><Users className="h-6 w-6" aria-hidden /><p className="mt-4 text-xs text-brass-ink">More seats. More stories.</p><h2 style={{ fontFamily: "var(--font-sans)", fontWeight: 600 }} className="mt-2 text-xl leading-snug">Dinner for the whole table</h2><p className="mt-3 text-sm text-ink-muted">Find family trays and generous portions to share.</p><span className="mt-4 inline-flex items-center gap-2 text-sm font-medium">Find family meals <ArrowRight className="h-4 w-4" aria-hidden /></span></button>
      <Link href="#community-favorites" className="min-w-[86%] shrink-0 snap-start rounded-2xl border border-line bg-forest-soft p-5 text-forest md:min-w-0 md:p-6"><Sparkles className="h-6 w-6" aria-hidden /><p className="mt-4 text-xs text-ink-muted">The neighborhood knows</p><h2 style={{ fontFamily: "var(--font-sans)", fontWeight: 600 }} className="mt-2 text-xl leading-snug">Meet your next regular spot</h2><p className="mt-3 text-sm text-ink-muted">Discover the kitchens diners keep coming back to.</p><span className="mt-4 inline-flex items-center gap-2 text-sm font-medium">See community favorites <ArrowRight className="h-4 w-4" aria-hidden /></span></Link>
    </div>
