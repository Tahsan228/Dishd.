...
export function ReviewComposer({ log, kitchen, dishes = [], onSaved }: { log: DiaryLog; kitchen?: { name: string; slug: string } | null; dishes?: ReviewDish[]; onSaved?: () => void }) {
  ...
  return <section className="rise rounded-3xl border border-line bg-surface p-6 sm:p-8" aria-label="Write your review"><p className="text-xs font-semibold uppercase tracking-widest text-brass-ink">Your meal, in your words</p><h2 className="mt-3 font-display text-3xl">How was that first bite?</h2><p className="mt-3 text-sm leading-7 text-ink-muted">Your pickup is verified. Share the flavors, the little details, and a photo worth a second helping.</p>
    <form onSubmit={submit} className="mt-7 space-y-6">
      <fieldset><legend className="text-sm font-semibold">Overall rating</legend><div className="mt-3 flex items-center justify-between gap-4 rounded-2xl bg-forest-soft p-5"><StarRating rating10={draft.rating === "" ? null : Number(draft.rating)} /><strong className="tabular text-xl text-forest">{draft.rating === "" ? "—" : Number(draft.rating) / 2}<span className="ml-1 text-xs font-normal">/ 5</span></strong></div>
        <div className="mt-3 grid grid-cols-5 gap-2">{[1,2,3,4,5].map(stars => <button key={stars} type="button" disabled={pending} aria-label={`Rate ${stars} out of 5 stars`} aria-pressed={Number(draft.rating) === stars * 2 && draft.rating !== ""} onClick={() => setDraft({ ...draft, rating: String(stars * 2) })} className={cn("flex min-h-12 items-center justify-center gap-1 rounded-xl border text-sm", Number(draft.rating) === stars * 2 ? "border-forest bg-forest-soft text-forest" : "border-line text-ink")}><Star className="h-4 w-4 fill-brass text-brass" aria-hidden />{stars}</button>)}</div>
        <label className="mt-3 block text-xs text-ink-muted">Overall star rating<select name="rating" value={draft.rating} onChange={e => setDraft({ ...draft, rating: e.target.value })} className={field} disabled={pending} required><option value="" disabled>Choose a rating</option>{Array.from({ length: 11 }, (_, i) => <option key={i} value={i}>{i / 2} stars</option>)}</select></label>{state.errors?.rating && <p className="mt-2 text-sm text-clay">{state.errors.rating}</p>}</fieldset>
      {dishes.length > 0 && <fieldset className="rounded-2xl border border-line p-4"><legend className="px-2 text-sm font-semibold text-forest">Rate each dish</legend><p className="text-xs text-ink-muted">Optional, and separate from the kitchen rating. Only dishes in this collected order can be rated.</p><div className="mt-4 space-y-4">{dishes.map(dish => <label key={dish.order_item_id} className="block text-sm font-medium">{dish.name}<select name={"dish_rating_" + dish.order_item_id} defaultValue={dish.rating_10 ?? ""} disabled={pending} className={field}><option value="">Not rated</option>{Array.from({ length: 11 }, (_, i) => <option key={i} value={i}>{i / 2} stars</option>)}</select></label>)}</div></fieldset>}
