...
export function PickupReviewPrompt({ initial }: { initial: PendingReview[] }) {
  ...
  return <>
    {reviews.length > 0 && <section aria-label="Meals ready to rate" className="mt-6 rounded-2xl border border-brass/40 bg-brass/10 p-5"><p className="flex items-center gap-2 text-sm font-semibold text-forest"><Star className="h-4 w-4 fill-brass text-brass" aria-hidden />Your last bite deserves a review</p><ul className="mt-3 space-y-3">{reviews.map(review => <li key={review.id} className="flex flex-wrap items-center justify-between gap-3"><span className="text-sm text-ink-muted">How was your pickup from <strong className="text-forest">{review.kitchen}</strong>?</span><button onClick={() => open(review)} className="min-h-11 rounded-full bg-forest px-5 text-sm font-medium text-cream">Rate this meal</button></li>)}</ul></section>}
