...
- Checkpoint checks: TypeScript passes; all 378 current unit tests and 80 isolated PostgreSQL checks pass (including Claude's timing tests). Initial schema activation status was pending; see the final verification below for the newer live observations.

### Discovery final verification

- Headless browser checks passed on the shared port 4173 preview using seeded accounts: all homepage sections, nine filters, search/clear, ZIP lookup, lazy map, video playback only after clicking Play, account dropdown, kitchen dish-rating labels, inline completed-order review, home review dialog and Escape dismissal, and owner discovery settings. Both 390px and 1280px fit without page overflow; no browser page errors. No reviews, orders, charges or seller declarations were submitted in live browser checks.
- Home reminders now show the latest pickup with older reminders collapsed. Offer banners swipe horizontally on mobile. Warm development renders measured approximately 1.1–1.4 seconds in four observed loads; these are local measurements, not a production latency guarantee. Screenshots and browser runners are ignored under .preview/.
- Live status has changed since the earlier notes: read-only Supabase checks now confirm kitchen_discovery_claims, menu_discovery, dish_rating_summaries, dish_ratings (including the stored menu_item_id), and orders.tip_cents exist. The authenticated dishd_reward_summary RPC works. The pickup-review RPC rejects a random nonexistent log with its expected completed-pickup guard. The browser renders dish-rating inputs and editable owner discovery controls. Codex did NOT apply a remote migration; these checks confirm the feature schema is now present, superseding the earlier pending-0013 note.
- Atomic review persistence and RLS enforcement are verified in isolated PostgreSQL. Live save mutations were deliberately not exercised against existing demo history. Stripe webhook/card settlement activation has not been reverified as part of discovery.
- Scoped ESLint passes without warnings. Earning formulas remain unchanged. The previously recorded signed-in header overflow is fixed by the account dropdown; older handoff entries saying it remains open are historical.
